...
describe("sqlite store — insert + dedup", () => {
  it("stores an event and reads it back", () => {
    const e = makeEvent({
      id: "evt-a",
      summary: "[example] Edit src/foo.ts",
      sessionId: "sess-1",
    ...
    expect(back?.summary).toBe("[example] Edit src/foo.ts");
...
describe("sqlite store — listProjects", () => {
  it("aggregates per project across agents and sessions", () => {
    ...
    const example = projects.find((p) => p.name === "example");
    expect(example).toBeDefined();
    expect(example?.eventCount).toBe(2);
    expect(example?.byAgent["claude-code"]).toBe(1);
    expect(example?.byAgent.codex).toBe(1);
    expect(example?.cost).toBeCloseTo(0.3);
    expect(example?.sessionIds.sort()).toEqual(["s1", "s2"]);
  });
});
